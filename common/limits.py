...
def GetObsLimitBisection( poi_bracket_init,
                          tolerance,
                          maxiter,
                          data,
                          model,
    ...
    for poi in poi_bracket:
        ...
        cls_obs_exp.append( pyhf.infer.hypotest( poi,
                                                 data,
                                                 model,
                                                 test_stat = "qtilde",
    ...
    niter = 1
    while ( poi_bracket[1] - poi_bracket[0] ) / ( 0.5 * ( poi_bracket[1] + poi_bracket[0] ) ) > tolerance:
        niter += 1
        if niter > maxiter:
            raise ValueError("Reached max. iterations in limit computation.")
        poi_mean = 0.5 * ( poi_bracket[1] + poi_bracket[0] )
        logger.debug(f"Limit Bisection (iteration {niter}): using POI value of {poi_mean}.")
        cls_obs_exp_new = pyhf.infer.hypotest( poi_mean,
                                               data,
                                               model,
                                               test_stat="qtilde",
                                               return_expected_set=True,
...
def limit_customScan(
    ...
    Args:
        ...
        toleranceObs (float, optional): rel. tolerance in POI value for convergence to CLs=0.05 - observed limit,
            defaults to 0.01
    ...
    ###############################################################################################
    #
    # Assumption: CLs values fall monotonically as the poi increases. This is not true in general,
    #             so be sure that it's true in the case at hand.
    #
    ###############################################################################################
    
    pyhf.set_backend("pytorch", precision="64b") # much faster to run than numpy
    ...
    results_obs, poi_values_obs = GetObsLimitBisection( poi_bracket,
                                                        toleranceObs,
                                                        maxiterObs,
                                                        data,
                                                        model,
    ...
    #
    # Determine the poi range for expected limits including +/1 and +/-2 sigma bands
    #
    ...
    poi_values_exp = np.arange( scan_lowerBound,
                                scan_upperBound + scan_resolution,
                                scan_resolution )
    logger.debug( f"poi_values_exp = {poi_values_exp}" )
    results_exp = [ pyhf.infer.hypotest( poi,
                                         data,
                                         model,
                                         test_stat = "qtilde",
                                         return_expected_set = True,
                                         par_bounds = par_bounds,
                                         fixed_params = fix_pars ) for poi in poi_values_exp ]
    ...
    if max( expected_minus2sigma ) < 0.05 or min( expected_plus2sigma ) > 0.05:
        raise ValueError("Could not determine expected limit bands. POI range inappropriate.")
    
    all_limits = []
    all_limits.append(np.interp(0.05, observed[::-1], poi_values_obs[::-1]))
    all_limits.append(np.interp(0.05, expected_minus2sigma[::-1], poi_values_exp[::-1]))
    all_limits.append(np.interp(0.05, expected_minus1sigma[::-1], poi_values_exp[::-1]))
    all_limits.append(np.interp(0.05, expected[::-1], poi_values_exp[::-1]))
    all_limits.append(np.interp(0.05, expected_plus1sigma[::-1], poi_values_exp[::-1]))
    all_limits.append(np.interp(0.05, expected_plus2sigma[::-1], poi_values_exp[::-1]))
